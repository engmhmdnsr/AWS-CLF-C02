...
window.startExam = function(num, isInteractive) {
  ...
  var qs = pool.map(function(q) {
    var tipEn = q.tip_en || 'Correct answer: <b>' + q.a_en + '</b>.';
    var tipAr = q.tip_ar || 'الإجابة الصحيحة: <b>' + q.a_ar + '</b>.';
    var aExp = q.a_explanation || tipEn;
    var opts = shuffle([
      { en: q.a_en, ar: q.a_ar, ok: true, expEn: aExp, expAr: tipAr },
      { en: q.distractors[0].en, ar: q.distractors[0].ar, ok: false, expEn: q.distractors[0].explanation || tipEn, expAr: tipAr },
      { en: q.distractors[1].en, ar: q.distractors[1].ar, ok: false, expEn: q.distractors[1].explanation || tipEn, expAr: tipAr },
      { en: q.distractors[2].en, ar: q.distractors[2].ar, ok: false, expEn: q.distractors[2].explanation || tipEn, expAr: tipAr }
    ]);
...
window.renderQ = function(idx) {
  ...
  var answered = ES.ans[idx] !== undefined;
  ...
  q.opts.forEach(function(opt, oi) {
    ...
    if (answered) {
      ...
      var expText = arOn ? opt.expAr : opt.expEn;
      var label = arOn ? (opt.ok ? '✅ صحيح. ' : '❌ خاطئ. ') : (opt.ok ? '✅ Correct. ' : '❌ Incorrect. ');
      var dirStyle = arOn ? 'direction:rtl;text-align:right;' : '';
      inner += '<div style="margin-top:10px;font-size:12px;background:' + bg + ';padding:9px 11px;border-radius:6px;border-left:3px solid ' + bc + ';color:var(--t);' + dirStyle + '">' + label + expText + '</div>';
    }
    inner += '</div>';
    div.innerHTML = inner;
